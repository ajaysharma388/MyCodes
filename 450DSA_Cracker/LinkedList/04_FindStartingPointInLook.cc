...
void findStartingPointOfLoopInLinkedList(Node *head) {
	Node *loopNode = findCycleNode(head);
	if(loopNode == NULL) { return; }
	...
	ptr = head;
	if(ptr == kthNode) {
		cout << "Starting Point of the loop is " << head->data << endl;
	}
	else {
    	...
    	cout << "Starting Point of the loop is " << ptr->next->data << endl;
	}
}
...
int main() {
  ...
  addCycle(4, head);
  findStartingPointOfLoopInLinkedList(head);
  printlinklist(head);
  return 0;
}
